...
import webbrowser
# Store data 
df = pd.read_csv('/Users/luammiller/Desktop/Code/Portfolio Project 2/phyx_exs_data.csv')

# Create string with exercise qualities that allows u to compare  
def combine_qualities(data):
    return [data["Difficulty"][i] + " "+ data[ "Body Part"][i] + " "+ data["Equipment"][i] for i in range(data.shape[0])]

# Create a column to hold combined qualities 
df["combined_qualities"] = combine_qualities(df)

# Convert combined qualites string into token matrix
...
#Introcduce the program an dchoose reccommendation method 
def intro():
    print("Welcome to the Phyx Exercise Prescription Algorithm! \n")
    reccommendation_method = None
    while reccommendation_method not in ["1", "2"]:
        reccommendation_method = input("You can recieve reccommendations based on:\n1 An exercise you like\n2 The part of the body you'd like to work\nType the number of the option you'd like to select: ")
        if reccommendation_method not in ["1", "2"]:
            print(f"\n'{reccommendation_method}' isn't an option. Type '1' or '2'\n")
    if reccommendation_method == "1":
        print_score()
        try_again()
    elif reccommendation_method == "2":
...
def get_input():
    ...
    exs_id = int(input("Type its number of the exercise to see similar ones:"))
...
def print_score():
    ...
    # Create enumerated list of similr exercises 
...
# Add reccommendation by category functionality 
body_part_column = df["Body Part"]

# Create list of body parts for user to chose from
def listify_strings(coulmn):
    return [i.split(",") for i in coulmn]
...
def get_body_part():
    ...
    # If the user does not enter a valid letter, prompt agan with a list of options
    if j < 0:
    ...
    while region_choice not in range(len(region_with_first_letter)):
        try: region_choice = int(input("Type the number of the body part you'd like t work."))
...
def display_exercises_in_region(region):
    rows_to_keep = [i for i in range(len(cleaned_regions_column)) if region in cleaned_regions_column[i]]
    exs_to_display = df.drop([i for i in range(df.shape[0]) if i not in rows_to_keep])
    exs_to_display = exs_to_display.drop(["exs_id", "regions_listed", "combined_qualities", "Body Part"], 1)
...
def see_videos():
    ...
    if choice == 'y':
        webbrowser.open('https://www.youtube.com/playlist?list=PLpqyTnl8cO1m3kL5goWP45PKkz1QfxRwF')
    if choice == 'n':
